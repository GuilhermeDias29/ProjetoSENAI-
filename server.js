...
app.post("/login", (req, res) => {
    ...
    db.get("SELECT * FROM usuario WHERE email = ? AND cpf = ?", [email, cpf], (err, row) => {
        if (row) res.json({ mensagem: "Ok", usuario: row });
        else res.status(401).json({ mensagem: "Erro" });
    });
});
...
// 2. LISTAR TODAS AS VAGAS (Para a visualizacao.html)
...
// 3. PEGAR UMA VAGA ESPECÍFICA (Para a descricao.html)
